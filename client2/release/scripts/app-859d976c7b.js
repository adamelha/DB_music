...
(function () {
  ...
  'BlurAdmin.pages.songs', 'BlurAdmin.pages.artists', 'BlurAdmin.pages.albums', 'BlurAdmin.pages.playlists']).config(routeConfig);
...
(function () {
  ...
  angular.module('BlurAdmin.pages.albums', []).config(routeConfig);
...
(function () {
  ...
  routeConfig.$inject = ["$stateProvider", "$urlRouterProvider"];
  angular.module('BlurAdmin.pages.artists', []).config(routeConfig);
...
(function () {
  ...
  routeConfig.$inject = ["$stateProvider", "$urlRouterProvider"];
  angular.module('BlurAdmin.pages.playlists', []).config(routeConfig);
...
(function () {
  ...
  routeConfig.$inject = ["$stateProvider", "$urlRouterProvider"];
  angular.module('BlurAdmin.pages.songs', []).config(routeConfig);
...
angular.module('BlurAdmin', ['ngAnimate', 'ui.bootstrap', 'ui.sortable', 'ui.router', 'ngTouch', 'toastr', 'Alertify', 'smart-table', "xeditable", 'ui.slimscroll', 'ui.select', 'ngStorage', 'ngJsTree', 'angular-progress-button-styles', 'BlurAdmin.connection', 'BlurAdmin.theme', 'BlurAdmin.auth', 'BlurAdmin.pages']);
(function () {
  'use strict';

  angular.module('BlurAdmin.auth').controller('loginPageCtrl', ['$scope', 'editableOptions', 'editableThemes', 'ServerConnection', '$state', 'Alertify', '$localStorage', function ($scope, editableOptions, editableThemes, ServerConnection, $state, Alertify, $localStorage) {

    $scope.signIn = function () {
      var inserted = getUserAndPassInput();
...
(function () {
  ...
  angular.module('BlurAdmin.auth').controller('signupPageCtrl', ['$scope', 'editableOptions', 'editableThemes', 'ServerConnection', '$state', 'Alertify', '$localStorage', function ($scope, editableOptions, editableThemes, ServerConnection, $state, Alertify, $localStorage) {
...
(function () {
  ...
  .factory('ServerConnection', ['$http', '$rootScope', '$q', '$localStorage', function ($http, $rootScope, $q, $localStorage) {
    var connection = {
      isLoading: false,
      serverUrl: 'http://delta-tomcat-vm.cs.tau.ac.il:40171', //TODO add port

      dataToUrl: function dataToUrl(data) {
        var andUri = "?";
        for (var key in data) {
      ...
      sendGet: function sendGet(route) {
        var data = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
        var cache = arguments[2];
        var successFunc = arguments[3];
        var errorFunc = arguments[4];

        this.showLoading();
        var uri = this.dataToUrl(data);
        return this.sendHttp('GET', null, this.serverUrl + route + uri, cache, successFunc, errorFunc);
      },
      sendPost: function sendPost(route) {
        var data = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
        var routedata = arguments[2];
        var cache = arguments[3];
        var successFunc = arguments[4];
        var errorFunc = arguments[5];

        this.showLoading();
        if (routedata) {
          var uri = this.dataToUrl([routedata]);
          return this.sendHttp('POST', {}, this.serverUrl + route + uri, cache, successFunc, errorFunc);
        } else return this.sendHttp('POST', _extends({}, data, { username: $localStorage.User.username, password: $localStorage.User.password }), this.serverUrl + route, cache, successFunc, errorFunc);
      },
      sendDelete: function sendDelete(route) {
        var data = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
        var cache = arguments[2];
        var successFunc = arguments[3];
        var errorFunc = arguments[4];
      ...
      sendPut: function sendPut(route) {
        var data = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
        var cache = arguments[2];
        var successFunc = arguments[3];
        var errorFunc = arguments[4];
      ...
      sendHttp: function sendHttp(method, data, url, cache, successFunc, errorFunc, upload) {
        var self = this;
        var deferred = $q.defer();
        var successFunction = function successFunction(response) {
          self.hideLoading();
          if (successFunc) successFunc(response.data, deferred);else {
        ...
        var errorFunction = function errorFunction(response) {
          self.hideLoading();
          if (errorFunc) errorFunc(response.data, deferred);else {
...
(function () {
  ...
  angular.module('BlurAdmin').directive('smartTable', smartTable);
  ...
  function smartTable() {
    return {
      ...
      controller: ["$scope", "$transclude", "editableOptions", "editableThemes", "$timeout", "ServerConnection", "$q", function controller($scope, $transclude, editableOptions, editableThemes, $timeout, ServerConnection, $q) {
        $scope.tableState = {
          items: [],
          pageSize: 10,
          page: 1
        ...
        $scope.isButtonsTranscludePresent = function () {
          return $transclude.isSlotFilled('buttons');
        };

        //

        $scope.isColumnEditable = function (colIndex) {
          return !(!$scope.editEnable || $scope.fields[colIndex].readOnly);
        };

        $scope.rowButtonClicked = function (item) {
          if ($scope.rowButton) {
        ...
        $scope.getFieldValue = function (item, field) {
          if (field.multiLanguage && $scope.languageId) {
            var tran = item.translations.filter(function (tran) {
              return tran.languageId == $scope.languageId;
            })[0];
            return tran && tran.texts ? tran.texts[field.name] : null;
          }

          var retVal = void 0;
          var val = $scope.getFieldByPath(item, field.name);
          switch (field.type) {
            case 'multiSelect':
              retVal = val ? val.map(function (val) {
        ...
        $scope.addCheckedItemToList = function (index) {
          $scope.setOfFunctionsFields.checkMarkList.push(index);
        };

        $scope.hiddenFilter = function (field) {
          return !field.tableHidden;
        };

        $scope.toggleFieldFilter = function (field, $event) {
        ...
        $scope.getDataFromServer = function (tableCtrl, a, field) {
          ...
          var paging = getPagination(tableCtrl.pagination);
          var sort = getSort(tableCtrl.sort);
          var filters = Object.assign({}, $scope.tableConfig.options.filter, tableCtrl.filter);
          var populate = $scope.tableConfig.options.populate;
          var reqOtions = _extends({}, paging, sort, { filters: _extends({}, filters) });
          $scope.tableConfig.reqOtions = reqOtions;
          //
          var reqResult = void 0;
        ...
        $scope.$on('itemsUpdated', function (e, res) {
        ...
        function updateFilter(tableCtrl, field) {
          ...
          if (field.filter.value === -1 || field.filter.value === "" || field.filter.value === null) {
            ...
            switch (field.type) {
              case 'select':
                tableCtrl.filter[field.name] = field.filter.options.filter(function (op) {
                  return op.key == field.filter.value;
                })[0].value;
                break;
              case 'boolean':
                tableCtrl.filter[field.name] = $scope.booleanFilterOptions.filter(function (op) {
                  return op.key == field.filter.value;
                })[0].key;
                break;
              case 'date':
                //TODO if needed, change format to YYYY-MM-DD
                var _field$filter$value = field.filter.value,
                    from = _field$filter$value.from,
                    to = _field$filter$value.to;
...
(function () {
  ...
  albumsCtrl.$inject = ["$scope", "$filter", "editableOptions", "editableThemes"];
  angular.module('BlurAdmin.pages.albums').controller('albumsCtrl', albumsCtrl);
...
(function () {
  ...
  artistsCtrl.$inject = ["$scope", "$filter", "editableOptions", "editableThemes"];
  angular.module('BlurAdmin.pages.artists').controller('artistsCtrl', artistsCtrl);
...
(function () {
  ...
  expandedPlaylistCtrl.$inject = ["$scope", "$state", "$filter", "editableOptions", "editableThemes", "ServerConnection", "$stateParams"];
  angular.module('BlurAdmin.pages.playlists').controller('expandedPlaylistCtrl', expandedPlaylistCtrl);
  ...
  function expandedPlaylistCtrl($scope, $state, $filter, editableOptions, editableThemes, ServerConnection, $stateParams) {
    ...
    $scope.leftColumn = {
      ...
      buttons: [{
        ...
        icon: 'fa fa-remove',
        isShownOnRow: function isShownOnRow() {
          return true;
        }
      }]
    };
...
(function () {
  ...
  playlistsCtrl.$inject = ["$scope", "$state", "$filter", "editableOptions", "editableThemes", "ServerConnection"];
  angular.module('BlurAdmin.pages.playlists').controller('playlistsCtrl', playlistsCtrl);
...
(function () {
  ...
  songsCtrl.$inject = ["$scope", "$filter", "editableOptions", "editableThemes", "$uibModal", "$timeout", "$q", "ServerConnection"];
  angular.module('BlurAdmin.pages.songs').controller('songsCtrl', songsCtrl);
  ...
  function songsCtrl($scope, $filter, editableOptions, editableThemes, $uibModal, $timeout, $q, ServerConnection) {
    ...
    $scope.text = {};
    $scope.updateSelect = function (playlist) {
    ...
    $scope.onChange = function (text) {
      ...
      //     returnMock()
      ServerConnection.sendPost('/searchPlaylist', { search: text }).then(function (r) {
    ...
    $scope.rowButton = {
      onClick: function onClick(row) {
        ...
        ServerConnection.sendPost('/singleLyrics', { filters: { track_id: row.track_id } }).then(function (r) {
          $scope.openRow = r;
    ...
    $scope.updateSongs = function (lyrics) {
      var options = $scope.tableConfig.reqOtions ? $scope.tableConfig.reqOtions : null;
      if (options) {
      ...
      ServerConnection.sendPost('/songs', options).then(function (r) {
        $scope.lyrics = '';
        $scope.songs = r.list;
...
angular.module('BlurAdmin').run(['$templateCache', function ($templateCache) {
  $templateCache.put('app/auth/login.html', '<title>DBmusic Login</title><link rel="icon" type="image/png" sizes="16x16" href="assets/img/favicon-16x16.png"><link rel="icon" type="image/png" sizes="32x32" href="assets/img/favicon-32x32.png"><link rel="icon" type="image/png" sizes="96x96" href="assets/img/favicon-96x96.png"><div class="auth-block"><h1>Sign in to DBmusic</h1><form class="form-horizontal"><div class="form-group"><label for="inputNamel3" class="col-sm-2 control-label">User Name</label><div class="col-sm-10"><input type="text" class="form-control login-input-size" id="inputNamel3" placeholder="Name"></div></div><div class="form-group"><label for="inputPassword3" class="col-sm-2 control-label">Password</label><div class="col-sm-10"><input type="password" class="form-control login-input-size" id="inputPassword3" placeholder="Password"></div></div><div class="form-group"><div class="col-sm-offset-2 col-sm-10"><button type="submit" ng-click="signIn()" class="btn btn-default btn-auth">Sign in</button><a ng-click="signUp()" class="forgot-pass">Sign Up</a></div></div></form></div>');
  $templateCache.put('app/auth/signup.html', '<div class="auth-block"><form class="form-horizontal"><div class="form-group"><label for="inputName3" class="col-sm-2 control-label">Name</label><div class="col-sm-10"><input type="text" class="form-control" id="inputName3" placeholder="Full Name"></div></div><div class="form-group"><label for="inputPassword3" class="col-sm-2 control-label">Password</label><div class="col-sm-10"><input type="password" class="form-control" id="inputPassword3" placeholder="Password"></div></div><div class="form-group"><div class="col-sm-offset-2 col-sm-10"><button ng-click="signUp()" type="submit" class="btn btn-default btn-auth">Sign up</button> <a ng-click="goToSignIn()" class="forgot-pass">Already have an account? Sign in!</a></div></div></form></div>');
  $templateCache.put('app/pages/albums/albums.html', '<div class="horizontal-scroll"><smart-table ng-if="tableConfig" panel-title="panelTitle" mock-items="items" table-config="tableConfig"></smart-table></div>');
  $templateCache.put('app/pages/artists/artists.html', '<div class="horizontal-scroll"><smart-table ng-if="tableConfig" panel-title="panelTitle" mock-items="items" table-config="tableConfig"></smart-table></div>');
  ...
  $templateCache.put('app/pages/maps/maps.html', '<div class="widgets"><div class="row"><div class="col-md-12" ui-view="" autoscroll="true" autoscroll-body-top=""></div></div></div>');
  $templateCache.put('app/pages/playlists/expandedPlaylist.html', '<div class="horizontal-scroll"><smart-table left-column="leftColumn" ng-if="tableConfig" panel-title="panelTitle" row-button="rowButton" mock-items="items" table-config="tableConfig"></smart-table></div>');
  $templateCache.put('app/pages/playlists/playlists.html', '<div class="horizontal-scroll"><smart-table left-column="leftColumn" ng-if="tableConfig" panel-title="panelTitle" row-button="rowButton" mock-items="items" table-config="tableConfig"></smart-table></div>');
  ...
  $templateCache.put('app/pages/songs/addToPlaylist.modal.html', '<div class="modal-content"><div class="modal-header"><button type="button" class="close" ng-click="$dismiss()" aria-label="Close"><em class="ion-ios-close-empty sn-link-close"></em></button><h4 class="modal-title">Add To Playlist</h4></div><div class="modal-body"><ui-select ng-model="text.searchText" class="show-tick" ng-change="updateSelect(text.searchText)" append-to-body="false" search-enabled="true" name="Playlist"><ui-select-match placeholder="{{ \'Select Option\'}}"><span>{{$select.selected.value}}</span></ui-select-match><ui-select-choices ng-hide="!$select.open" repeat="option in playlists" refresh="onChange($select.search)" refresh-delay="0"><span ng-bind-html="option.name"></span></ui-select-choices></ui-select></div><div class="modal-footer"><div style="display: flex;"><input type="text" ng-model="text.newPlaylistName" class="form-control"> <button type="button" class="btn btn-primary" ng-click="updateSelect({name:text.newPlaylistName})">Add To New Playlist</button></div></div></div>');
  $templateCache.put('app/pages/songs/songs.html', '<div class="horizontal-scroll"><smart-table right-column="rightColumn" ng-if="tableConfig" panel-title="panelTitle" mock-items="items" row-button="rowButton" table-config="tableConfig"><buttons><div style="display: flex; width: 30%;"><input ng-model="lyrics" name="Search Lyrics" type="text" class="form-control"> <button class="btn btn-default btn-auth" ng-click="updateSongs(lyrics)">Search Lyrics</button></div></buttons></smart-table></div>');
  $templateCache.put('app/directives/smart-table/smart-table.html', '<div class="row smart-table" ng-init="init()"><div class="col-md-12"><div ba-panel="" ba-panel-title="{{panelTitle}}" ba-panel-class="with-scroll"><div ng-if="isButtonsTranscludePresent()" class="add-row-editable-table"><ng-transclude class="transcludedButtons" ng-transclude-slot="buttons"></ng-transclude></div><div class="horizontal-scroll"><div class="form-group select-page-size-wrap"><label>Items Per Page<select class="form-control show-tick" title="Items Per Page" selectpicker="" ng-model="tableState.pageSize" ng-options="i for i in [1,5,10,15,20,25]"></select></label></div><table st-pipe="getDataFromServer" class="table" st-table="tableState.items"><thead><tr class="sortable columnTitle"><th ng-if="setOfFunctionsFields"></th><th ng-if="leftColumn">{{leftColumn.name}}</th><th ng-repeat="field in fields | filter: hiddenFilter" st-skip-natural="true" st-sort="{{field.name}}" ng-if="!field.isHidden" class="headerField"><span class="tabName">{{field.alias}}<span class="fieldFilter" ng-click="toggleFieldFilter(field,$event)"><i class="fa fa-filter" aria-hidden="true"></i></span></span><div ng-click="stopProp($event)" ng-show="field.filter.open" ng-switch="field.type" class="filterWrapper"><div ng-switch-when="select|boolean" ng-switch-when-separator="|"><div class=""><ui-select ng-model="field.filter.value" class="btn-group bootstrap-select form-control" ng-change="getDataFromServer(tableCtrl,null,field)" append-to-body="false" search-enabled="false" name="{{field.name}}"><ui-select-match placeholder="{{field.filter.placeholder || \'Select Option\'}}"><span>{{$select.selected.value}}</span></ui-select-match><ui-select-choices ng-hide="!$select.open" repeat="option.key as option in (field.type==\'boolean\'? booleanFilterOptions: field.filter.options) | filter: $select.search"><span ng-bind-html="option.value"></span></ui-select-choices></ui-select></div></div><div class="dateWrapper" ng-switch-when="date"><div><div>From :</div><bb-datepicker min-date="minDate" hide-time="true" class="date form-control" date-time="field.filter.value.from"></bb-datepicker></div><div><div>To:</div><bb-datepicker hide-time="true" class="date form-control" date-time="field.filter.value.to"></bb-datepicker></div><div><i class="fa fa-search" ng-click="getDataFromServer(tableCtrl,null,field)"></i></div></div><div ng-switch-default=""><input ng-model="field.filter.value" name="{{field.name}}" type="{{(field.type == \'integer\' || field.type == \'number\') ? \'number\' : (field.type ? field.type : \'text\')}}" class="form-control" ng-required="field._required" ng-class="{\'input-disabled\':!field.editable}" ng-change="updateFilter(tableCtrl,field)"> <i class="fa fa-search" ng-click="getDataFromServer(tableCtrl,null,field)"></i></div></div></th></tr><tr ng-if="searchGlobal"><th colspan="6"><input st-search="" class="form-control" placeholder="Search..." type="text"></th></tr><tr ng-if="searchEnable"><th ng-if="setOfFunctionsFields"></th><th ng-if="leftColumn"></th><th ng-repeat="field in fields | filter: hiddenFilter"><input st-search="{{field.name}}" placeholder="Search {{field.alias}}" class="input-sm form-control search-input" type="search"></th></tr></thead><tbody ng-show="tableState.isLoading"><tr><td colspan="4" class="text-center">Loading ...</td></tr></tbody><tbody ng-show="!tableState.isLoading"><tr class="table-data" ng-class="rowButton ? \'st-row\': \'\'" ng-repeat="item in tableState.items | filter: setOfFiltersFields.currentFilter(item)"><td ng-if="setOfFunctionsFields"><label class="checkbox-inline custom-checkbox nowrap"><input ng-click="addCheckedItemToList(item.id)" ng-model="options.closeButton" type="checkbox" id="closeButton"> <span></span></label></td><td ng-if="leftColumn.name"><div ng-if="btn.isShownOnRow(item)" class="buttonList" ng-repeat="btn in leftColumn.buttons" ng-click="btn.onClick(item,$parent.$parent.$index)"><i ng-class="btn.icon" aria-hidden="true"></i></div></td><td ng-if="!isColumnEditable($index)" ng-click="rowButtonClicked(item)" ng-class="{\'firstChild\': !$index && item.parent}" ng-repeat="field in fields | filter: hiddenFilter">{{getFieldValue(item,field)}}</td><td ng-if="isColumnEditable($index)" ng-click="rowButtonClicked(item)" ng-repeat="field in fields | filter: hiddenFilter"><span editable-text="getFieldValue" blur="cancel">{{getFieldValue(item,field)}}</span></td><td ng-if="rightColumn.name"><div ng-if="btn.isShownOnRow(item)" class="buttonList" ng-repeat="btn in rightColumn.buttons" ng-click="btn.onClick(item,$parent.$parent.$index)"><i ng-class="btn.icon" aria-hidden="true"></i></div></td></tr></tbody><tfoot ng-if="tableState.pageSize"><tr><td colspan="5" class="text-center"><div st-pagination="" st-items-by-page="tableState.pageSize" st-displayed-pages="7"></div></td></tr></tfoot></table></div></div></div></div>');
  ...
  $templateCache.put('app/theme/components/pageTop/pageTop.html', '<div class="page-top clearfix" scroll-position="scrolled" max-height="50" ng-class="{\'scrolled\': scrolled}"><a href="#/dashboard" class="al-logo clearfix"><span>DB</span>music</a> <a href="" class="collapse-menu-link ion-navicon" ba-sidebar-toggle-menu=""></a></div>');
...
//# sourceMappingURL=../maps/scripts/app-859d976c7b.js.map
